...
var displayCards = {
    ...
    player: () => {
        ...
        console.log(Random.userHit, Random.dealerHit);
        let user_len = Random.userHit.length;
        let currToInt = Random.userHit[user_len - 1];
        let player_game_hit = new Game_Card_Process(currToInt);
        player_game_hit.BeginUserHit();
    },
    dealer: () => {
        console.log('stand is pressed');
        let dealer_len = Random.dealerHit.length;
        let currToInt = Random.dealerHit[dealer_len - 1];
...
var hitstand = {
    ...
    standPressed: () => {
        Func.display = 'dealer';
        document.querySelector('.hit-stand-button').style.visibility = 'hidden';
        console.log(Random.dealerTotalCardValue);
        var init = new computer(Random.userTotalCardValue, Random.dealerTotalCardValue);
        init.make_move();
    },
};
...
/////////////////////////////////////////////////////////
function bet(e) {
    let chipVal = parseInt(e.target.value);
...
function handleEvent(e) {
    switch (e) {
        ...
        case 'next':
            ...
            document.querySelector('.next').style.visibility =
                'hidden';
            document.querySelector('.hit-stand-button').style.visibility = 'visible';
            break;
        default:
